...
public class Block implements Collidable, Sprite, HitNotifier {
     ...
     */
    public Block(Rectangle r, Color c, int hp) {
        this.blockShape = r;
    ...
    public Velocity hit(Ball hitter, Point collisionPoint, Velocity currentVelocity) {
        //make variables of the collision edges
        double xLeft = getCollisionRectangle().getUpperLeft().getX();
        double xRight = getCollisionRectangle().getBottomRight().getX();
        double yUp = getCollisionRectangle().getUpperLeft().getY();
        double yDown = getCollisionRectangle().getBottomRight().getY();
        //epsilon to check deviation
        double eps = 0.0007;
        //if the x collision point is within the x values of the rectangle make dx as -dx
        if ((Math.abs(collisionPoint.getX() - xLeft) <= eps) || (Math.abs(collisionPoint.getX() - xRight) <= eps)) {
            currentVelocity = new Velocity(-currentVelocity.getDx(), currentVelocity.getDy());
        }
        //if the x collision point is within the x values of the rectangle make dx as -dx
        if ((Math.abs(collisionPoint.getY() - yUp) <= eps) || (Math.abs(collisionPoint.getY() - yDown) <= eps)) {
            currentVelocity = new Velocity(currentVelocity.getDx(), -currentVelocity.getDy());
        }
        //make new velocity variable
        Velocity newVel = currentVelocity;
        //check if the ball was hit and if it was reduce the hit points number
        if (this.hitNum > 0) {
